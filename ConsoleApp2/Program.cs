...
class Program
{
    ...
    static bool CheckIfObjectExists(int id, string connectionString)
    {
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            using (SqlCommand command = new SqlCommand("SELECT COUNT(*) FROM Shop WHERE Id = @Id", connection))
            {
    ...
    static void InsertIntoDatabase(ApiObject apiObject, string connectionString)
    {
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            using (SqlCommand command = new SqlCommand("INSERT INTO Shop (Id, Title, Body) VALUES (@Id, @Title, @Body)", connection))
            {
